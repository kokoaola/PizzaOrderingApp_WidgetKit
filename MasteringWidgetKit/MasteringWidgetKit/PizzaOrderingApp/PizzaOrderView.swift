...
struct PizzaOrderView: View{
                    ...
                    Section(header: Text("Summary")) {
                        ...
                        Button {
                            ...
                            HStack {
                                ...
                                // 注文ボタン
                                Text("Place order")
                                    .foregroundColor(!currentID.isEmpty ? .gray : .blue)
                            }
                        }
                    }
    ...
    private func placeOrder(orderString: String) {
        ...
        // コンテンツを生成し、staleDateを指定する
        // 1時間後に古くなると設定
        let staleDate = Date().addingTimeInterval(60 * 60)
        let content = ActivityContent(state: state, staleDate: staleDate)
        
        
        do {
            // アクティビティを要求して注文情報を保存
            let activity = try Activity<PizzaOrderAttributes>.request(attributes: pizzaOrderAttrs, content: content, pushType: nil)
    ...
    private func updateOrder(with status: OrderStatus) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            // 新しい状態を生成
            let newState = PizzaOrderAttributes.ContentState(status: status)
